...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        assert cur_place is not None
        count = 0
        while count < self.lower_bound and not cur_place.is_hive:
            cur_place = cur_place.entrance
            count += 1

        while (
            not cur_place.is_hive and not cur_place.bees and count <= self.upper_bound
        ):
            cur_place = cur_place.entrance
            count += 1

        if cur_place.is_hive or not (
            self.lower_bound <= count and count <= self.upper_bound
        ):
            return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        health = self.health
        attack = health + self.damage if health <= amount else amount
        place = self.place
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def new_action(gamestate):
            if (target.slow_time and gamestate.time % 2 == 0) or not target.slow_time:
