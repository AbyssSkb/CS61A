...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        lambda_expressions = expressions.map(lambda x: x)
        lambda_expressions.first = lambda_expressions.first.rest
        env.define(signature.first, do_lambda_form(lambda_expressions, env))
