def shuffle(s):
    ...
    for i in range(length // 2):
        ...
        new_list.append(s_list[i + length // 2])
...
def is_planet(p):
    ...
    return isinstance(p, list) and len(p) == 2 and p[0] == "planet"
...
def max_path_sum(t):
    ...
    max_sum = weight
    for branch in branches(t):
        max_sum = max(max_sum, max_path_sum(branch) + weight)
...
def is_mobile(m):
    ...
    return isinstance(m, list) and len(m) == 3 and m[0] == "mobile"
...
def is_arm(s):
    ...
    return isinstance(s, list) and len(s) == 3 and s[0] == "arm"
